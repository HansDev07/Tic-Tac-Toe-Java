...
public class MainActivity extends AppCompatActivity {
    ...
    private void onButtonClick(View view) {
        ...
        roundCount++;
        if (checkWin()) { //if true x scores if false o scores
            if (playerXTurn) {
                scoreX++;
                Toast.makeText(this, "Player X Wins!", Toast.LENGTH_SHORT).show(); //note: tried toast notification
            } else {
                scoreO++;
    ...
    private boolean checkWin() {
        String[][] field = new String[3][3]; //made a new temporary array to assign current board state to
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                field[i][j] = buttons[i][j].getText().toString();
            }
        }
        // Check rows & columns for matches
        for (int i = 0; i < 3; i++) {
    ...
    private void resetBoard() { //round count back to 0 and resets board
        roundCount = 0;
        playerXTurn = true; //X back to true X always go first
        for (int i = 0; i < 3; i++) { //runs through the buttons and sets all marked buttons with empty text
            for (int j = 0; j < 3; j++) {
                buttons[i][j].setText("");
            }
        }
    }
    private void resetScore() { //resets score counter including board
        scoreX = 0;
        scoreO = 0;
        updateScore();
        resetBoard();
